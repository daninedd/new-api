...
func modelPriceNotConfiguredError(modelName string, userId int) error {
	if model.IsAdmin(userId) {
		return fmt.Errorf(
			"Model %s price is not configured. Enable self-use mode in System Settings -> Operation Settings, or configure pricing for this model in System Settings -> Group and Model Pricing Settings. "+
				"Model %s price not configured. Go to System Settings → Operation Settings to enable self-use mode, or configure the model price in System Settings → Group & Model Pricing.",
			modelName, modelName,
		)
	}
	return fmt.Errorf(
		"Model %s price has not been configured by the administrator and is temporarily unavailable. Please contact the site administrator to enable this model. "+
