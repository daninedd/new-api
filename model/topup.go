...
func UpdatePendingTopUpStatus(tradeNo string, expectedPaymentProvider string, targetStatus string) error {
	if tradeNo == "" {
		return errors.New("Payment order number was not provided")
	}
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	if tradeNo == "" {
		return errors.New("Order number was not provided")
	}
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.Status != common.TopUpStatusPending {
			return errors.New("The order status is not pending payment, so it cannot be completed manually")
		}
	...
	// 事务外记录日志，避免阻塞
	RecordTopupLog(userId, fmt.Sprintf("Manual order completion successful, top-up amount: %v, payment amount: %f", logger.FormatQuota(quotaToAdd), payMoney), callerIp, paymentMethod, "admin")
	return nil
}
