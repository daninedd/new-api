...
autoDetect:
	...
	if host == "" {
		return nil, fmt.Errorf("Unable to determine Passkey Origin. Specify it in system settings or Passkey settings. Current Host: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
