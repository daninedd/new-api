...
func BuildWebAuthn(r *http.Request) (*webauthn.WebAuthn, error) {
	...
	if settings == nil {
		return nil, errors.New("Passkey settings were not found")
	}
	...
	if originsStr != "" {
		...
		for _, origin := range originList {
			...
			if !settings.AllowInsecureOrigin && strings.HasPrefix(strings.ToLower(trimmed), "http://") {
				return nil, fmt.Errorf("Passkey does not allow insecure origins: %s", trimmed)
			}
...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf("Passkey only supports HTTPS. Current access: %s://%s，please allow insecure origins in Passkey settings or configure HTTPS", scheme, r.Host)
	}
	// 优先使用请求的完整Host（包含端口）
	host := r.Host
	...
	if host == "" {
		return nil, fmt.Errorf("Unable to determine the Passkey origin. Please specify it in system settings or Passkey settings. Current Host: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("Passkey origin is not configured, so the RPID cannot be derived")
	}
