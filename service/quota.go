...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if totalTokens == 0 {
		...
		quota = 0
		logContent += fmt.Sprintf(" (possibly an upstream timeout)")
	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}, current remaining quota is {{value}}. To avoid interrupting your usage, please top up in time.<br/>Top-up link: <a href='{{value}}'>{{value}}</a>"
