...
func parseJSONArray(jsonStr string, typeName string) ([]map[string]interface{}, error) {
	...
	if err := json.Unmarshal([]byte(jsonStr), &list); err != nil {
		return nil, fmt.Errorf("Invalid %s format: %s", typeName, err.Error())
	}
	return list, nil
}
...
func validateURL(urlStr string, index int, itemType string) error {
	if !urlRegex.MatchString(urlStr) {
		return fmt.Errorf("The URL format of %s #%d is invalid", itemType, index)
	}
	if _, err := url.Parse(urlStr); err != nil {
		return fmt.Errorf("The URL of %s #%d cannot be parsed: %s", itemType, index, err.Error())
	}
	return nil
}
...
func validateApiInfo(apiInfoStr string) error {
	...
	for i, apiInfo := range apiInfoList {
		...
		if len(urlStr) > 500 {
			return fmt.Errorf("API info #%d URL cannot exceed 500 characters", i+1)
		}
		if len(route) > 100 {
			return fmt.Errorf("API info #%d route description cannot exceed 100 characters", i+1)
		}
		if len(description) > 200 {
			return fmt.Errorf("API info #%d description cannot exceed 200 characters", i+1)
		}

		if !validColors[color] {
			return fmt.Errorf("API info #%d color value is invalid", i+1)
		}
...
func validateAnnouncements(announcementsStr string) error {
	...
	for i, ann := range list {
		...
		if _, err := time.Parse(time.RFC3339, publishDateStr); err != nil {
			return fmt.Errorf("Announcement #%d publish date format is invalid", i+1)
		}
		...
		if len(content) > 500 {
			return fmt.Errorf("Announcement #%d content cannot exceed 500 characters", i+1)
		}
		if extra, exists := ann["extra"]; exists {
			if extraStr, ok := extra.(string); ok && len(extraStr) > 200 {
				return fmt.Errorf("Announcement #%d description cannot exceed 200 characters", i+1)
			}
		}
	}
	return nil
...
func validateFAQ(faqStr string) error {
	...
	for i, faq := range list {
		...
		if len(question) > 200 {
			return fmt.Errorf("FAQ #%d question cannot exceed 200 characters", i+1)
		}
		if len(answer) > 1000 {
			return fmt.Errorf("FAQ #%d answer cannot exceed 1000 characters", i+1)
		}
	}
	return nil
}
...
func validateUptimeKumaGroups(groupsStr string) error {
	...
	for i, group := range groups {
		...
		if nameSet[categoryName] {
			return fmt.Errorf("Group #%d category name duplicates another group", i+1)
		}
		...
		if len(categoryName) > 50 {
			return fmt.Errorf("Group #%d category name cannot exceed 50 characters", i+1)
		}
		if len(urlStr) > 500 {
			return fmt.Errorf("Group #%d URL cannot exceed 500 characters", i+1)
		}
		if len(slug) > 100 {
			return fmt.Errorf("Group #%d Slug cannot exceed 100 characters", i+1)
		}
		if len(description) > 200 {
			return fmt.Errorf("Group #%d description cannot exceed 200 characters", i+1)
		}
