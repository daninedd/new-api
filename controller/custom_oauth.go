...
func FetchCustomOAuthDiscovery(c *gin.Context) {
	...
	if wellKnownURL == "" && issuerURL == "" {
		common.ApiErrorMsg(c, "Please enter the Discovery URL or Issuer URL first")
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, "Failed to get Discovery configuration: "+err.Error())
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		common.ApiErrorMsg(c, "Failed to get Discovery configuration: "+message)
		return
	}
...
func DeleteCustomOAuthProvider(c *gin.Context) {
	...
	if count > 0 {
		common.ApiErrorMsg(c, "This OAuth provider still has user bindings and cannot be deleted. Please remove all user bindings first.")
		return
	}
