...
func UniversalVerify(c *gin.Context) {
	...
	case "passkey":
		...
		if err != nil {
			common.ApiError(c, fmt.Errorf("Passkey verification status is abnormal: %v", err))
			return
		}
		if !verified {
	...
	// 记录日志
	model.RecordLog(userId, model.LogTypeSystem, fmt.Sprintf("General security verification succeeded (method: %s)", verifyMethod))
