...
func SubscriptionRequestStripePay(c *gin.Context) {
	...
	if plan.StripePriceId == "" {
		common.ApiErrorMsg(c, "This plan has no StripePriceId configured")
		return
	}
	...
	payLink, err := genStripeSubscriptionLink(referenceId, user.StripeCustomer, user.Email, plan.StripePriceId, plan.PriceAmount)
	if err != nil {
...
func genStripeSubscriptionLink(referenceId string, customerId string, email string, priceId string, priceAmount float64) (string, error) {
	stripe.Key = setting.StripeApiSecret

	params := &stripe.CheckoutSessionParams{
		ClientReferenceID: stripe.String(referenceId),
		SuccessURL: stripe.String(paymentReturnPath(fmt.Sprintf(
			"/console/topup?show_history=true&pay=success&transaction_id=%s&value=%s&currency=USD",
			url.QueryEscape(referenceId),
			url.QueryEscape(strconv.FormatFloat(priceAmount, 'f', 2, 64)),
		))),
