...
func Setup2FA(c *gin.Context) {
	...
	if existing != nil && existing.IsEnabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The user has already enabled 2FA. Please disable it before setting it up again",
		})
		return
	}
	...
	// 记录操作日志
	model.RecordLog(userId, model.LogTypeSystem, "Started two-step verification setup")

	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "2FA setup initialized successfully. Please scan the QR code with an authenticator app and enter the verification code to complete setup",
...
func Disable2FA(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(userId, model.LogTypeSystem, "Disabled two-step verification")
...
func RegenerateBackupCodes(c *gin.Context) {
	...
	// 记录操作日志
	model.RecordLog(userId, model.LogTypeSystem, "Regenerated two-step verification backup codes")
...
func AdminDisable2FA(c *gin.Context) {
	...
	if myRole <= targetUser.Role && myRole != common.RoleRootUser {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "You do not have permission to modify 2FA settings for users at the same or a higher level",
		})
		return
	}
	...
	model.RecordLogWithAdminInfo(userId, model.LogTypeManage,
		"Administrator forcibly disabled the user's two-step verification", adminInfo)
