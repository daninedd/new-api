...
func RequestEpay(c *gin.Context) {
	...
	if client == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Payment information has not been configured by the administrator"})
		return
	}
...
func EpayNotify(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("EPay webhook request received path=%q client_ip=%s method=%s params=%q", c.Request.RequestURI, c.ClientIP(), c.Request.Method, common.GetJsonString(params)))
	...
	if err == nil && verifyInfo.VerifyStatus {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("EPay webhook signature verified successfully trade_no=%s callback_type=%s trade_status=%s client_ip=%s verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.TradeStatus, c.ClientIP(), common.GetJsonString(verifyInfo)))
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if isNonEpayPaymentMethodForEpayCallback(topUp.PaymentMethod) {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("EPay order payment method does not match trade_no=%s order_payment_method=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentMethod, verifyInfo.Type, c.ClientIP()))
			return
		}
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("EPay top-up successful trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, topUp.Money, common.GetJsonString(topUp)))
