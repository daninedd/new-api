...
func StripeWebhook(c *gin.Context) {
	...
	callerIp := c.ClientIP()
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook signature verified successfully event_type=%s client_ip=%s path=%q", string(event.Type), callerIp, c.Request.RequestURI))
	switch event.Type {
...
func sessionCompleted(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if "complete" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.completed status is abnormal, ignoring trade_no=%s status=%s client_ip=%s", referenceId, status, callerIp))
		return
	}

	paymentStatus := event.GetObjectValue("payment_status")
	if paymentStatus != "paid" {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe Checkout payment is not complete, waiting for async result trade_no=%s payment_status=%s client_ip=%s", referenceId, paymentStatus, callerIp))
		return
	}
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failure event is missing order number client_ip=%s", callerIp))
		return
	}
	...
	if topUp.PaymentMethod != model.PaymentMethodStripe {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failed but order payment method does not match trade_no=%s payment_method=%s client_ip=%s", referenceId, topUp.PaymentMethod, callerIp))
		return
	}
	...
	logger.LogInfo(ctx, fmt.Sprintf("Stripe top-up order has been marked as failed trade_no=%s client_ip=%s", referenceId, callerIp))
}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe completed order is missing order number client_ip=%s", callerIp))
		return
	}
	...
	currency := strings.ToUpper(event.GetObjectValue("currency"))
	logger.LogInfo(ctx, fmt.Sprintf("Stripe top-up successful trade_no=%s amount_total=%.2f currency=%s event_type=%s client_ip=%s", referenceId, total/100, currency, string(event.Type), callerIp))
}
...
func sessionExpired(ctx context.Context, event stripe.Event) {
	...
	status := event.GetObjectValue("status")
	if "expired" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.expired status is abnormal, ignoring trade_no=%s status=%s", referenceId, status))
		return
	}

	if len(referenceId) == 0 {
		logger.LogWarn(ctx, "Stripe checkout.expired is missing order number")
		return
	}
	...
	if errors.Is(err, model.ErrTopUpNotFound) {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe top-up order does not exist and cannot be marked expired trade_no=%s", referenceId))
		return
	}
	if err != nil {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Stripe top-up order has expired trade_no=%s", referenceId))
}
