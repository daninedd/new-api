...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(fmt.Sprintf(
		"Upstream model check summary: checked channels %d, changed channels %d, added models %d, removed models %d, auto-added models %d, failed channels %d.",
		checkedChannels,
		changedChannels,
		detectedAddModels,
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		builder.WriteString(fmt.Sprintf("\n\nAdded model samples (showing %d/%d): %s",
			displayCount,
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		builder.WriteString(fmt.Sprintf("\n\nRemoved model samples (showing %d/%d): %s",
			displayCount,
...
func runChannelUpstreamModelUpdateTaskOnce() {
	...
	if changedChannels > 0 || failedChannels > 0 {
		...
		service.NotifyUpstreamModelUpdateWatchers(
			"Upstream model check notification",
