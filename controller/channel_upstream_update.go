...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(fmt.Sprintf(
		"Upstream model inspection summary: checked %d channels, found changes in %d, added %d, removed %d, auto-synced %d new models, failed %d.",
		checkedChannels,
		changedChannels,
		detectedAddModels,
	...
	if len(channelSummaries) > 0 {
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(fmt.Sprintf("\n- %d other channels omitted", len(channelSummaries)-displayCount))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		builder.WriteString(fmt.Sprintf("\n\nNew model examples (showing %d/%d): %s",
			displayCount,
		...
		if len(normalizedAddModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (%d others omitted)", len(normalizedAddModelSamples)-displayCount))
		}
	}
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		if len(normalizedRemoveModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (%d others omitted)", len(normalizedRemoveModelSamples)-displayCount))
		}
	}
	...
	if failedChannels > 0 {
		...
		if failedChannels > displayCount {
			builder.WriteString(fmt.Sprintf(" (%d others omitted)", failedChannels-displayCount))
		}
	}
