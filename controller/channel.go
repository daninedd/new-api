...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if err := channel.ValidateSettings(); err != nil {
		return fmt.Errorf("Channel extra setting format error: %s", err.Error())
	}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Batch adding Vertex AI keys must use a valid JSON array format, for example [{key1}, {key2}...]. Please check the input: %w", err)
	}
	...
	if len(cleanKeys) == 0 {
		return nil, fmt.Errorf("Vertex AI keys cannot be empty for batch add")
	}
...
func ManageMultiKeys(c *gin.Context) {
	...
	case "disable_all_keys":
		...
		if disabledCount == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No keys can be disabled",
			})
			return
		}
	...
	case "delete_disabled_keys":
		...
		if deletedCount == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No automatically disabled keys need to be deleted",
			})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf("Deleted %d automatically disabled keys", deletedCount),
			"data":    deletedCount,
		})
		return
