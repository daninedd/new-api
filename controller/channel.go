...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if channel.Type == constant.ChannelTypeVertexAi {
		...
		if regionMap["default"] == nil {
			return fmt.Errorf("Deployment region must contain the default field")
		}
	}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Batch adding Vertex AI must use a valid JSON array format, for example[{key1}, {key2}...]，please check the input: %w", err)
	}
...
func CopyChannel(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "Failed to get channel information. Please try again later"})
		return
	}
...
func ManageMultiKeys(c *gin.Context) {
	...
	case "disable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No key index was specified to disable",
			})
			return
		}
	...
	case "enable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No key index was specified to enable",
			})
			return
		}
	...
	case "enable_all_keys":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf("%d keys have been enabled", enabledCount),
		})
		return
	...
	case "disable_all_keys":
		...
		if disabledCount == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "There are no keys that can be disabled",
			})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf("%d keys have been disabled", disabledCount),
		})
		return
	...
	case "delete_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No key index was specified to delete",
			})
			return
		}
		...
		if len(remainingKeys) == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "The last key cannot be deleted",
			})
			return
		}
	...
	case "delete_disabled_keys":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf("%d auto-disabled keys have been deleted", deletedCount),
			"data":    deletedCount,
		})
		return
